...
from scene.CRiM import CRiMKernel
from utils.general_utils import safe_state
import uuid
from tqdm import tqdm
from utils.image_utils import psnr
from argparse import ArgumentParser, Namespace
from arguments import ModelParams, PipelineParams, OptimizationParams, CRiMParams
try:
...
def training(dataset, opt, pipe, crimopt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    first_iter = 0
    ...
    crimkernel = CRiMKernel(num_views=len(trainCameras),
                            view_dim=crimopt.view_dim,
                            num_warp=crimopt.num_warp,
                            method=crimopt.method,
                            adjoint=crimopt.adjoint,
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration > crimopt.start_warp:
            
            image_ori = image
            warped_cams, ortho_loss = crimkernel.get_warped_cams(viewpoint_cam)
            rendered_images = []
            for cam in warped_cams:
            ...
            if iteration > crimopt.start_pixel_weight:
                rendered_images = torch.stack(rendered_images, dim=0)
                rendered_images_down = torch.nn.functional.interpolate(rendered_images, 
                                                                 size=(viewpoint_cam.image_height // 2, viewpoint_cam.image_width // 2),
                                                                 mode='bilinear', align_corners=False)
                pixel_weights, pixel_mask = crimkernel.get_weight_and_mask(rendered_images_down.detach(), viewpoint_cam.uid)
                image = torch.sum(pixel_weights * rendered_images, dim=0)
                image = image * pixel_mask + image_ori * (1 - pixel_mask) + 1e-10
                pixel_mask_loss = pixel_mask.mean()

            else:
                rendered_images.append(image_ori)
                rendered_images = torch.stack(rendered_images, dim=0)
                image = torch.mean(rendered_images, dim=0) + 1e-10
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                if iteration > crimopt.start_warp:
                    crimkernel.optimizer.step()
                    crimkernel.optimizer.zero_grad()
                    crimkernel.adjust_lr()
...
if __name__ == "__main__":
    ...
    cp = CRiMParams(parser)
    parser.add_argument('--ip', type=str, default="127.0.0.1")
    parser.add_argument('--port', type=int, default=6009)
    parser.add_argument('--debug_from', type=int, default=-1)
    parser.add_argument('--detect_anomaly', action='store_true', default=False)
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3_000, 6_000, 9_000, 12_000, 15_000, 18_000, 21_000, 24_000, 27_000, 30_000, 33_000, 36_000, 39_000, 40_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 6_000, 9_000, 12_000, 15_000, 18_000, 21_000, 24_000, 27_000, 30_000, 33_000, 36_000, 39_000, 40_000])
