...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor, gamma_correction):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        lpips_scores.append(lpips_fn(rendering, gt).cpu().item())
...
def render_video(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor, gamma_correction):
    ...
    with torch.no_grad():
        ...
        if gamma_correction:
            rgbs = [to8b(render_pkg["render"].permute(1, 2, 0).cpu().numpy() ** (1 / 2.2)) for render_pkg in render_pkgs]
        else:
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video : bool, pose_optimize : bool, gamma_correction : bool):
    ...
    with torch.no_grad():
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, kernel_size, scale_factor=scale_factor, gamma_correction=gamma_correction)
        if not skip_video:
            render_video(dataset.model_path, "video", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, kernel_size, scale_factor=scale_factor, gamma_correction=gamma_correction)

    if not skip_test:
        with torch.no_grad():
            print("Rendering Test Set Before Pose Optimization")
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, kernel_size, scale_factor=scale_factor, gamma_correction=gamma_correction)

        if pose_optimize: # We use the pose optimization code of Deblur-GS
            initialize_test_pose(dataset, scene, gaussians, background, exclude=[], old_version=False, gamma_correction=gamma_correction)
            fit_cams = optimize_test_pose(scene, gaussians, pipeline=pipeline, bg_color=background, kernel_size=kernel_size, gamma_correction=gamma_correction)
            with torch.no_grad():
                print("\nRendering Test Set After Pose Optimization")
                render_set(dataset.model_path, "test", scene.loaded_iter, fit_cams, gaussians, pipeline, background, kernel_size, scale_factor=scale_factor, gamma_correction=gamma_correction)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_video, args.pose_optimize, args.gamma_correction)
